...
locales=('ar' 'bg' 'da-DK' 'de-DE' 'es-ES' 'el-GR' 'fr-FR' 'it-IT' 'nl-NL' 'pl-PL' 'pt-BR' 'ru-RU' 'sv-SE' 'ta-IN' 'tr-TR' 'uk' 'zh-CN' 'zh-TW')
...
for l in "${locales[@]}"; do
	...
	for i in $(seq 0 4); do
		...
		elif [ -d ./app/src/main/res/drawable-"${l:0:2}"r"${l:2:}" ] ; then
			cp -f -r ./fastlane/metadata/android/"${l}"/images/phoneScreenshots/"${screenshots[$i]}".png ./app/src/main/res/drawable-"${l:0:2}"r"${l:2:}"/intro_"${names[$i]}".png
		else
