...
/**
 * The wire protocol shared with the `:wear` companion app. The two sides live in separate Gradle
 * modules (separate installable APKs), so these constants and [WatchReminderItem] are duplicated
 * on the watch side (`WearProtocol.kt` there) rather than shared via a dependency - keep the two
...
/** Interface so `foss` can bind a no-op implementation and stay free of Play Services Wearable. */
