...
def escape(str_xml: str):
    # lxml handles standard XML escaping automatically.
    # We only need to handle Android-specific escapes like apostrophes.
    return str_xml.replace("'", "\\'")
...
def load_english_strings() -> typing.Dict[str, str]:
    ...
    tree = ET.parse("core/ui/src/main/res/values/strings.xml", parser)
...
def load_language_trees(
    ...
    for directory in glob.glob("core/ui/src/main/res/*"):
        if directory == "core/ui/src/main/res/values" or not os.path.exists(
            directory + "/strings.xml"
        ):
            continue
        language = get_language_from_directory(directory)
        if language == "nb-rNO" or language == "is":
            continue
        language_tree[language] = ET.parse(directory + "/strings.xml", parser)
...
def write_language_trees(
    ...
    for language in languages:
        tree = language_tree[language]
        ET.indent(tree, space="    ")
        with open(f"core/ui/src/main/res/values-{language}/strings.xml", "wb") as f:
...
def translate_with_deepl(
    ...
    for string_name, language in translate_list:
        ...
        _set_string_translation(language_tree[language], string_name, translated.text)


def _set_string_translation(
    tree: lxml.etree._ElementTree, string_name: str, translation: str
) -> None:
    element = tree.find(f'string[@name="{string_name}"]')
    if element is None:
        element = ET.SubElement(tree.getroot(), "string", name=string_name)
...
def translate_with_claude(
    ...
    for language, string_names in sorted(by_language.items()):
        translations = _fetch_claude_batch(language, string_names, english_strings)
        if not translations:
            continue
        ...
        for string_name in string_names:
            ...
            if _validate_format_specifiers(
                english_strings[string_name], translation, string_name
            ):
                _set_string_translation(
                    language_tree[language], string_name, translation
                )


def _fetch_claude_batch(
    language: str,
    string_names: typing.List[str],
    english_strings: typing.Dict[str, str],
    ...
    return _parse_claude_json(result.stdout, language)
